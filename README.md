...
A pré-consulta de placa/chassi usa um Chromium headless no servidor (o site de consulta só aceita navegador real). O `Dockerfile` (usado pelo Railway, ver `railway.json`) parte do Debian e instala o Chromium pelo apt, já com `CHROME_PATH=/usr/bin/chromium`. Em outro ambiente, defina `CHROME_PATH` apontando para o Chrome/Chromium. O log de boot mostra `[pré-consulta] Chromium: …` ou os caminhos tentados; sem Chromium, a pré-consulta avisa "site barrou a consulta" e o resto do sistema segue normal.
